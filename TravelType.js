...
const TravelType = ({ goToPage }) => {
  ...
  return (
    <div
      ...
      style={{ backgroundImage: 'url(https://media.istockphoto.com/id/1142873622/vector/travel.jpg?s=612x612&w=0&k=20&c=IzNwO_x-ENHTLfuObj5NFITXapRxgaMlgfVLqYjGT-0=' }}
    >
      <h1 className="text-4xl font-bold text-black mb">Choose Your Travel Type</h1>
      <h2 className="mb-8 text-2xl text-black">Choose Who You Are?</h2>
      <div className="grid grid-cols-2 gap-4 p-8 bg-white rounded-lg bg-opacity-30 glassmorphism">
        {travelTypes.map((type) => (
          <button
            key={type.id}
            className={`flex items-center p-4 bg-white bg-opacity-50 rounded-lg text-left hover:scale-105 hover:shadow-lg transition transform duration-200 ${selected === type.id ? 'bg-blue-200' : ''}`}
            onClick={() => handleSelect(type.id)}
          >
            <img src={type.image} alt={type.title} className="object-cover w-16 h-16 mr-4 rounded-full" />
      ...
      <button
          className="px-4 py-2 text-white bg-gray-500 rounded hover:bg-black"
          onClick={() => goToPage && goToPage('/tripDetails')}
        >
          Back
        </button>
        <button
          className="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-700"
          onClick={() => goToPage('nextPage')} // Adjust this if the next page has a different route
        >
          Next
        </button>
      </div>
...
const TravelType = ({ goToPage }) => {
  ...
  const handleSelect = (id) => {
    setSelected(selected === id ? null : id); // Toggle selection (deselect if already selected)
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-center bg-cover" style={{ backgroundImage: 'url(https://media.istockphoto.com/id/473778812/photo/funny-cat-with-passport-and-airline-ticket.jpg?s=612x612&w=0&k=20&c=Mm1jZCnpiNsOdOBlPOxv1LREPEERG5lD70GoO9Gu5Pg=' }}>
      <div className="w-full max-w-lg p-8 bg-white bg-opacity-50 rounded-lg shadow-lg mt-1/3 mb-1/3">
        ...
        <div className="flex justify-center p-4 mt-6 space-x-4 rounded-lg bg-opacity-30">
          <button className="px-4 py-2 text-white bg-gray-500 rounded hover:bg-black" onClick={() => goToPage('tripDetails')}>Back</button>
          <button className="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-700">Next</button>
        </div>
      </div>
    </div>
  );
